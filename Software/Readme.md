...
- **Gaze & Pose:** Identifies the target person and tracks facial landmarks to determine gaze direction, then smoothly maps it to servo angles via a critically-damped smoothing filter.
...
- **Name Correction:** A `PostSTTProcessor` corrects misheard Bangla proper nouns before they reach the AI. It uses Levenshtein distance, SequenceMatcher, and semantic embeddings to accurately look up teacher names in the school database.
- **Audio Management:** Precisely timed audio streaming loops prevent the microphone from capturing the robot's own Text-to-Speech output (audio feedback).
...
- **RAG Knowledge Base:** Structured JSON records of school staff and institutional data, indexed by role and name using sentence-transformer semantic search for rapid answers without internet reliance.
...
- **Optimization:** Text inputs are cleaned of mixed Unicode. Repeated phrases are cached as hashed audio to provide instant responses for frequent greetings.

## Branches Reference
Check the following branches on the main repository for software implementations:
